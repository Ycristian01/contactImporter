  ...
  def self.import(file)
    CSV.foreach(file.path, headers: true) do |row|
      ...
      contact = find_or_create_by!(name: contact_hash['name'], dayOfBirth: contact_hash['dayOfBirth'], phone: contact_hash['phone'],
        address: contact_hash['address'], card: contact_hash['card'], email: contact_hash['email'])
      contact.update(contact_hash)
    end
    table = CSV.parse(file.read, headers: true)
    puts table.headers
    # puts "ESTE ES EL VALOR DEL FILE EN 0" + table.by_row[0].to_s
    # puts  "ESTE ES EL VALOR DEL FILE EN 0" + table.by_row[1].to_s
  end
end
